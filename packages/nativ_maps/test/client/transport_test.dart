...
void main() {
  ...
  group('reintentos', () {
    test('reintenta un 503 y acaba respondiendo', () async {
      ...
      final maps = fakeNativMaps(service);
  ...
  group('errores', () {
    ...
    test('usar el cliente después de close lanza StateError', () async {
      final maps = fakeNativMaps(service)..close();
  ...
  group('autenticación', () {
    ...
    test('el proxy manda a su host y dice qué servicio era', () async {
      ...
      expect(peticion.headers['X-Nativ-Service'], 'geo-places');
      expect(peticion.headers['X-Nativ-Region'], 'us-east-1');
    });
